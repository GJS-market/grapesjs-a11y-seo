// Demo sandbox: a deliberately "bad" page that trips most rules, so you can see
// the auditor light up and try Show / Fix / Learn and the SEO tab.
//
// Query params:
...
const engine = params.get('engine') === 'axe' ? 'axe' : 'builtin';
const stress = Math.max(0, parseInt(params.get('stress') || '0', 10) || 0);
const extraPages = Math.min(50, Math.max(0, parseInt(params.get('pages') || '0', 10) || 0));
...
const editor = grapesjs.init({
  ...
  // Seed the Asset Manager with a few offline images so the SEO tab's image
  // pickers (JSON-LD image/logo, OG image) have something to choose.
  assetManager: {
    assets: [
      swatch('#5b8cff', 'Hero'),
      swatch('#22c55e', 'Team'),
